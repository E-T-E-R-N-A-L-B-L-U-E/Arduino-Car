...
//			delay( round( to_down_rate * ( current_position - pos ) ) );
			run( round( to_down_rate * ( current_position - pos ) ), -1 );
		} else {
			onMoving = false;
			return ;
		}
...
//	elevator_left.run( RELEASE );
//	elevator_right.run( RELEASE );
	run( current_position - goal, -1 );
	current_position = goal;
	onMoving = false;
}

// this function should only be used privately
void ElevatorController::run( const int &runtime, const int &direct ) {
	if ( runtime <= 0 ) {
		return;
	}
	if ( direct == 1 ) {
